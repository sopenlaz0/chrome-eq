...

{
  var core = {
    ...
    equalizer: {
          ...
          if (o.eq && o.ch) {
            if (core.equalizer.filters.length > 0) {
              if (core.equalizer.filters[0]) {
                core.equalizer.filters[0].channelCount = o.ch.mono
                  ? 1
                  : core.equalizer.filters[0]._defaultChannelCount
                  ? core.equalizer.filters[0]._defaultChannelCount
                  : 2;
        ...
        if (core.equalizer.source && core.equalizer.context) {
          if (core.equalizer.filters && core.equalizer.filters.length) {
            if (core.equalizer.storage.state === "ON") {
              core.equalizer.source.disconnect(
                core.equalizer.context.destination
              );
      ...
      load: function () {
        if (core.log)
          console.error(" >> Searching for video/audio elements...");
          ...
          core.equalizer.targets.forEach(function (t, k) {
            if (t.getAttribute("equalizer-state") !== "attached") {
              ...
              if (src) {
                ...
                //
                try {
                  core.equalizer.source =
                    core.equalizer.context.createMediaElementSource(t);
                  core.equalizer.filters[0]._defaultChannelCount = core
                    .equalizer.source.channelCount
                    ? core.equalizer.source.channelCount
                    : 2;
                  let target =
                    core.equalizer.storage.state === "OFF"
                      ? core.equalizer.context.destination
                      : core.equalizer.filters[0];
                  ...
                  //
                  let last =
                    core.equalizer.filters[core.equalizer.filters.length - 1];
                  last.connect(core.equalizer.context.destination);
                  window.top.postMessage(
                    { app: "audio-equalizer", action: "app-icon-normal" },
                    "*"
                  );
                } catch (e) {
                  window.top.postMessage(
                    { app: "audio-equalizer", action: "app-icon-error" },
                    "*"
                  );
                }
              }
